...
public class BoardDAO extends JDBConnect {
    ...
    public int insertWrite(String tname, BoardDTO dto) {
        int result = 0;        
        try {
    ...
    public BoardDTO selectView(String tname, String num) { 
        ...
        String query = "SELECT B.*, M.name " 
                     + " FROM membership M INNER JOIN "+tname+" B " 
                     + " ON M.id=B.id "
                     + " WHERE num=?";
        try {
    ...
    //게시물의 조회수를 1 증가시킨다. 
    public void updateVisitCount(String tname, String num) {
    ...
    //게시물 삭제하기 
    public int deletePost(String tname,BoardDTO dto) { 
        int result = 0;
    ...
    public List<BoardDTO> selectListPage(Map<String, Object> map) {
        ...
        String query = " SELECT * FROM ( "
                    ...
                    + "        SELECT B.*, M.name FROM "+ map.get("tname")+" B inner join membership M on M.id=B.id ";
